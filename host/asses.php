

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="CoreUI - Open Source Bootstrap Admin Template">
    <meta name="author" content="Łukasz Holeczek">
    <meta name="keyword" content="Bootstrap,Admin,Template,Open,Source,AngularJS,Angular,Angular2,Angular 2,Angular4,Angular 4,jQuery,CSS,HTML,RWD,Dashboard,React,React.js,Vue,Vue.js">
    <link rel="shortcut icon" href="data:image/png;base64,">
    <meta name="csrf-token" content="x1ZDz3mvfdo17z6Leyn4i2XuMECMow1vCUWlNtm0">

    <title>HOST SUPERVISOR</title>
    <link href="include/datatable.css" rel="stylesheet">
    <link href="include/select1.css" rel="stylesheet">
    <link href="include/fonts.css" rel="stylesheet">
    <link href="include/icons.css" rel="stylesheet">
    <link href="include/style.css" rel="stylesheet">
    <link href="include/date.css" rel="stylesheet">
    <link href="include/custom.css" rel="stylesheet" >
</head>

<body class="app header-fixed sidebar-fixed aside-menu-fixed aside-menu-hidden">
    <header class="app-header navbar">
        <button class="navbar-toggler mobile-sidebar-toggler d-lg-none mr-auto" type="button">
                    <span class="navbar-toggler-icon"></span>
                </button>
        <a class="navbar-brand" href="#"></a>
        <button class="navbar-toggler sidebar-toggler d-md-down-none" type="button">
                    <span class="navbar-toggler-icon"></span>
                </button>

        <ul class="nav navbar-nav d-md-down-none">
            <li class="nav-item px-3">
                <a class="nav-link" href="#">Dashboard</a>
            </li>
        </ul>
        <span class="navbar-text ml-auto">
                    <strong>

            <?php echo $host_name; ?> <b class="caret" ></b></strong>
                </span>
        <ul class="nav navbar-nav ml-auto">
            <i class*="icon-user" ></i>

            <?php echo $host_name; ?> <b class="caret" ></b> 
            
            <li class="nav-item d-md-down-none">
                <button data-toggle="modal" data-target="#modal-logout" class="btn btn-link"><span class="fa fa-sign-out"></span>Logout </button>                
            </li>
        </ul>
    </header>


    <div class="app-body" id="app">
        <div class="modal fade" id="modal-logout" tabindex="-1" role="dialog" aria-labelledby="Logout" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                        <div class="modal-header">
                            <p class="modal-title">Confirm Portal Logout</p>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                        </div>
                    <div class="modal-body">
                        <p>This will log you out and end all your sessions. Do you wan to continue?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                         <form action="host_login.php" method="POST">
                             <input type="hidden" name="_token" value="d8iOj6D4jRpgVZNB9UQZL04fyN0o9JoEISIxcn6R">                         <button type="submit" class="btn btn-danger">Yes, Log me out</button>
                        </form>
                        
                    </div>
                </div>
            </div>            
        </div>

        <div class="sidebar">
            <nav class="sidebar-nav">
                <ul class="nav">
                    <li class="nav-item">
                        <a class="nav-link" href="index.php?dashboard"><i class="icon-graph"></i> Dashboard </a>
                    </li>

                                         <li class="nav-item">
                        <a class="nav-link" href="interns.php"><i class="icon-list"></i> Interns </a>
                    </li>
                      
                    

                                                         </ul>
            </nav>
            <button class="sidebar-minimizer brand-minimizer" type="button"></button>
        </div>
        <main class="main">
              
<div class="card">
	<div class="card-header">
		<h4><strong> Man, Spider (11111) Attachment @ Madaraka Primary School</strong>

	</div>
	<div class="card-body">
								<button data-toggle="modal" data-target="#modal-delete-assessment" class="btn btn-danger btn-sm pull-right">Delete Assessment</button>
							<div class="row justify-content-center">
                    <div class="col-8">
                        <h3 class="text-center text-uppercase"><b>SBL Host Supervisor Evaluation Form</b></h3>
                        <hr>
                        <p class="text-justify">The internship period is intended to run for at least 225 hours after which the student is to be graded based on the Student report, Host Supervisor evaluation and Faculty Supervisor evaluation. To facilitate this, please provide constructive feedback to both the student and the organisation. Part A consists of rating questions while Part B contains open ended questions.</p>

                                                <h4><b>Part A: Rating Questions</b></h4>

                        <h5><b>Ranking Criteria</b></h5>
                        <dl class="list-unstyled">
                                                        <li><b>1.00</b> = Below expectations: Did not exert any unusual effort on the job. Did minimal work.</li>
                                                        <li><b>2.00</b> = Needed constant supervision: Overcame some of the obstacles encountered on the job and undertook some of the required tasks. Needed constant supervision</li>
                                                        <li><b>3.00</b> = Met expectations: Overcame normal obstacles encountered on the job and undertook the required tasks. Needed average supervision.</li>
                                                        <li><b>4.00</b> = Needed minimal supervision: Overcame obstacles encountered on the job and worked beyond the requirements. Needed minimal supervision. </li>
                                                        <li><b>5.00</b> = Exceeded expectations: Overcame obstacles encountered on the job and demonstrated extraordinary talent and/or effort. Made suggestions for improvements.</li>
                                                    </dl>

                        <div class="table-responsive">
                            <table class="table">
                                                                <tr>
                                                                        <td><b>Personal Skills</b></td>
                                                                        <td class="text-center"><b>1.00</b></td>
                                                                        <td class="text-center"><b>2.00</b></td>
                                                                        <td class="text-center"><b>3.00</b></td>
                                                                        <td class="text-center"><b>4.00</b></td>
                                                                        <td class="text-center"><b>5.00</b></td>
                                                                                                        </tr>
                                                                <tr>
                                                                        <td>Demonstrates competency in the skills needed to work in community service</td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="30" id="11"
                                                   value="11"
                                                   aria-label="Below expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="30" id="12"
                                                   value="12"
                                                   aria-label="Needed constant supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="30" id="13"
                                                   value="13"
                                                   aria-label="Met expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="30" id="14"
                                                   value="14"
                                                   aria-label="Needed minimal supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="30" id="15"
                                                   value="15"
                                                   aria-label="Exceeded expectations"
                                                                                                       checked                                                                                                 >
                                        </div>
                                    </td>
                                                                                                        </tr>
                                                                <tr>
                                                                        <td>Displays appropriate oral and nonverbal communication skills</td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="31" id="11"
                                                   value="11"
                                                   aria-label="Below expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="31" id="12"
                                                   value="12"
                                                   aria-label="Needed constant supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="31" id="13"
                                                   value="13"
                                                   aria-label="Met expectations"
                                                                                                       checked                                                                                                 >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="31" id="14"
                                                   value="14"
                                                   aria-label="Needed minimal supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="31" id="15"
                                                   value="15"
                                                   aria-label="Exceeded expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                                                        </tr>
                                                                                                <tr>
                                                                        <td><b>Personal Outcomes</b></td>
                                                                        <td class="text-center"><b>1.00</b></td>
                                                                        <td class="text-center"><b>2.00</b></td>
                                                                        <td class="text-center"><b>3.00</b></td>
                                                                        <td class="text-center"><b>4.00</b></td>
                                                                        <td class="text-center"><b>5.00</b></td>
                                                                                                        </tr>
                                                                <tr>
                                                                        <td>Demonstrates an appreciation for community service</td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="32" id="11"
                                                   value="11"
                                                   aria-label="Below expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="32" id="12"
                                                   value="12"
                                                   aria-label="Needed constant supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="32" id="13"
                                                   value="13"
                                                   aria-label="Met expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="32" id="14"
                                                   value="14"
                                                   aria-label="Needed minimal supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="32" id="15"
                                                   value="15"
                                                   aria-label="Exceeded expectations"
                                                                                                       checked                                                                                                 >
                                        </div>
                                    </td>
                                                                                                        </tr>
                                                                <tr>
                                                                        <td>Functions as a productive team member within the organization</td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="33" id="11"
                                                   value="11"
                                                   aria-label="Below expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="33" id="12"
                                                   value="12"
                                                   aria-label="Needed constant supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="33" id="13"
                                                   value="13"
                                                   aria-label="Met expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="33" id="14"
                                                   value="14"
                                                   aria-label="Needed minimal supervision"
                                                                                                       checked                                                                                                 >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="33" id="15"
                                                   value="15"
                                                   aria-label="Exceeded expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                                                        </tr>
                                                                <tr>
                                                                        <td>Assimilates and uses supervisory feedback  to improve</td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="34" id="11"
                                                   value="11"
                                                   aria-label="Below expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="34" id="12"
                                                   value="12"
                                                   aria-label="Needed constant supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="34" id="13"
                                                   value="13"
                                                   aria-label="Met expectations"
                                                                                                       checked                                                                                                 >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="34" id="14"
                                                   value="14"
                                                   aria-label="Needed minimal supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="34" id="15"
                                                   value="15"
                                                   aria-label="Exceeded expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                                                        </tr>
                                                                <tr>
                                                                        <td>Demonstrates the willingness and ability to learn new skills</td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="35" id="11"
                                                   value="11"
                                                   aria-label="Below expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="35" id="12"
                                                   value="12"
                                                   aria-label="Needed constant supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="35" id="13"
                                                   value="13"
                                                   aria-label="Met expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="35" id="14"
                                                   value="14"
                                                   aria-label="Needed minimal supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="35" id="15"
                                                   value="15"
                                                   aria-label="Exceeded expectations"
                                                                                                       checked                                                                                                 >
                                        </div>
                                    </td>
                                                                                                        </tr>
                                                                <tr>
                                                                        <td>Arrives to work on time and meets deadlines (time management and dependability)</td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="36" id="11"
                                                   value="11"
                                                   aria-label="Below expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="36" id="12"
                                                   value="12"
                                                   aria-label="Needed constant supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="36" id="13"
                                                   value="13"
                                                   aria-label="Met expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="36" id="14"
                                                   value="14"
                                                   aria-label="Needed minimal supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="36" id="15"
                                                   value="15"
                                                   aria-label="Exceeded expectations"
                                                                                                       checked                                                                                                 >
                                        </div>
                                    </td>
                                                                                                        </tr>
                                                                <tr>
                                                                        <td>Projects a positive attitude towards their duties</td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="57" id="11"
                                                   value="11"
                                                   aria-label="Below expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="57" id="12"
                                                   value="12"
                                                   aria-label="Needed constant supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="57" id="13"
                                                   value="13"
                                                   aria-label="Met expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="57" id="14"
                                                   value="14"
                                                   aria-label="Needed minimal supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="57" id="15"
                                                   value="15"
                                                   aria-label="Exceeded expectations"
                                                                                                       checked                                                                                                 >
                                        </div>
                                    </td>
                                                                                                        </tr>
                                                                                                <tr>
                                                                        <td><b>Organizational fit</b></td>
                                                                        <td class="text-center"><b>1.00</b></td>
                                                                        <td class="text-center"><b>2.00</b></td>
                                                                        <td class="text-center"><b>3.00</b></td>
                                                                        <td class="text-center"><b>4.00</b></td>
                                                                        <td class="text-center"><b>5.00</b></td>
                                                                                                        </tr>
                                                                <tr>
                                                                        <td>Works effectively with a diverse workforce (e.g. age group, culture, gender, socio-economic status etc.) </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="37" id="11"
                                                   value="11"
                                                   aria-label="Below expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="37" id="12"
                                                   value="12"
                                                   aria-label="Needed constant supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="37" id="13"
                                                   value="13"
                                                   aria-label="Met expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="37" id="14"
                                                   value="14"
                                                   aria-label="Needed minimal supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="37" id="15"
                                                   value="15"
                                                   aria-label="Exceeded expectations"
                                                                                                       checked                                                                                                 >
                                        </div>
                                    </td>
                                                                                                        </tr>
                                                                <tr>
                                                                        <td>Observes the rules, policies and procedures of the organization</td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="38" id="11"
                                                   value="11"
                                                   aria-label="Below expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="38" id="12"
                                                   value="12"
                                                   aria-label="Needed constant supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="38" id="13"
                                                   value="13"
                                                   aria-label="Met expectations"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="38" id="14"
                                                   value="14"
                                                   aria-label="Needed minimal supervision"
                                                                                                                                                                                                      >
                                        </div>
                                    </td>
                                                                        <td class="text-center">
                                        <div class="form-check">
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <input required class="form-check-input position-static" type="radio"
                                                   name="38" id="15"
                                                   value="15"
                                                   aria-label="Exceeded expectations"
                                                                                                       checked                                                                                                 >
                                        </div>
                                    </td>
                                                                                                        </tr>
                                                                                            </table>
                        </div>

                                                <h4><b>Part B: Please complete the following section</b></h4>

                        
                        <dl class="list-unstyled">
                                                    </dl>

                        <div class="table-responsive">
                            <table class="table">
                                                                <tr>
                                                                    </tr>
                                                                <tr>
                                                                        <td colspan="2">
                                        <div class="form-group">
                                            <label for="description" class="form-label text-md-right"><b>Please comment on the intern’s overall performance with regards to identified strengths and/weaknesses.</b></label>
                                                                                        <textarea required id="39" class="form-control" name="39" rows="4" placeholder="Assessment Comments">his performance is above average. he is very hardworking and resourceful.</textarea>
                                                                                    </div>
                                    </td>
                                                                    </tr>
                                                                <tr>
                                                                        <td colspan="2">
                                        <div class="form-group">
                                            <label for="description" class="form-label text-md-right"><b>What were the specific contributions to the organisation from the student’s internship?</b></label>
                                                                                        <textarea required id="40" class="form-control" name="40" rows="4" placeholder="Assessment Comments">he assisted greatly in the schools IT section</textarea>
                                                                                    </div>
                                    </td>
                                                                    </tr>
                                                                <tr>
                                                                        <td colspan="2">
                                        <div class="form-group">
                                            <label for="description" class="form-label text-md-right"><b>With regards to the general performance and behaviour of the student, please identify (if any) specific virtues displayed by the student.</b></label>
                                                                                        <textarea required id="41" class="form-control" name="41" rows="4" placeholder="Assessment Comments">honesty and trustworthiness</textarea>
                                                                                    </div>
                                    </td>
                                                                    </tr>
                                                                <tr>
                                                                        <td colspan="2">
                                        <div class="form-group">
                                            <label for="description" class="form-label text-md-right"><b>Please provide any recommendations to Portal and/ student interns so as to improve the future internship programme.</b></label>
                                                                                        <textarea required id="42" class="form-control" name="42" rows="4" placeholder="Assessment Comments">n/a</textarea>
                                                                                    </div>
                                    </td>
                                                                    </tr>
                                                                                            </table>
                        </div>

                                                                        <a href="" class="btn btn-primary pull-right">Download Assessment</a>
                                            </div>

                </div>	</div>
</div>
<div class="modal fade" id="modal-delete-assessment" tabindex="-1" role="dialog" aria-labelledby="Delete Assessment" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">Delete Assessment</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				<p>Are you absolutely sure you want to delete this assessment?</p>
				<form method="POST" action="">
					<input type="hidden" name="_token" value="L4xv3w8oxeWJEo59Y1sfz4w0AYokJAcacYQj299d">	
					<input type="hidden" name="_method" value="DELETE">
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
						<button type="submit" class="btn btn-danger">Yes, I am sure</button>
					</div>
				</form>
			</div>

	
</div>
        </main>
    </div>
    <footer class="app-footer">
        <span>&copy; <a href="#" target="_blank">BBIT GROUP A</a> 2021.</span>
    </footer>

    <script src="scripts/app.js"></script>
    
    <script src="scripts/popper.min.js"></script>
    <script src="scripts/bootstrap.min.js"></script>
    <script src="scripts/pace.min.js"></script>
    <script src="scripts/Chart.min.js"></script>
    <script src="scripts/datatables.min.js"></script>
    <script src="scripts/select2.min.js"></script>
    <script src="scripts/.easy-autocomplete.js"></script>
    <script src="scripts//moment.js"></script>
    <script src="scripts/daterangepicker.js"></script>
    <script src="scripts/custom.js"></script>
    <script>
	$(function () {

	});
</script>
</body>

</html>