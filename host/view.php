
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="CoreUI - Open Source Bootstrap Admin Template">
    <meta name="author" content="Łukasz Holeczek">
    <meta name="keyword" content="Bootstrap,Admin,Template,Open,Source,AngularJS,Angular,Angular2,Angular 2,Angular4,Angular 4,jQuery,CSS,HTML,RWD,Dashboard,React,React.js,Vue,Vue.js">
    <link rel="shortcut icon" href="data:image/png;base64,">
    <meta name="csrf-token" content="x1ZDz3mvfdo17z6Leyn4i2XuMECMow1vCUWlNtm0">

    <title>HOST SUPERVISOR</title>
    <link href="include/datatable.css" rel="stylesheet">
    <link href="include/select1.css" rel="stylesheet">
    <link href="include/fonts.css" rel="stylesheet">
    <link href="include/icons.css" rel="stylesheet">
    <link href="include/style.css" rel="stylesheet">
    <link href="include/date.css" rel="stylesheet">
    <link href="include/custom.css" rel="stylesheet" >
</head>

<body class="app header-fixed sidebar-fixed aside-menu-fixed aside-menu-hidden">
    <header class="app-header navbar">
        <button class="navbar-toggler mobile-sidebar-toggler d-lg-none mr-auto" type="button">
                    <span class="navbar-toggler-icon"></span>
                </button>
        <a class="navbar-brand" href="#"></a>
        <button class="navbar-toggler sidebar-toggler d-md-down-none" type="button">
                    <span class="navbar-toggler-icon"></span>
                </button>

        <ul class="nav navbar-nav d-md-down-none">
            <li class="nav-item px-3">
                <a class="nav-link" href="#">Dashboard</a>
            </li>
        </ul>
        <span class="navbar-text ml-auto">
                    <strong>

            <?php echo $host_name; ?> <b class="caret" ></b></strong>
                </span>
        <ul class="nav navbar-nav ml-auto">
            <i class*="icon-user" ></i>

            <?php echo $host_name; ?> <b class="caret" ></b> 
            
            <li class="nav-item d-md-down-none">
                <button data-toggle="modal" data-target="#modal-logout" class="btn btn-link"><span class="fa fa-sign-out"></span>Logout </button>                
            </li>
        </ul>
    </header>


    <div class="app-body" id="app">
        <div class="modal fade" id="modal-logout" tabindex="-1" role="dialog" aria-labelledby="Logout" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                        <div class="modal-header">
                            <p class="modal-title">Confirm Portal Logout</p>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                        </div>
                    <div class="modal-body">
                        <p>This will log you out and end all your sessions. Do you wan to continue?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                         <form action="host_login.php" method="POST">
                             <input type="hidden" name="_token" value="d8iOj6D4jRpgVZNB9UQZL04fyN0o9JoEISIxcn6R">                         <button type="submit" class="btn btn-danger">Yes, Log me out</button>
                        </form>
                        
                    </div>
                </div>
            </div>            
        </div>

        <div class="sidebar">
            <nav class="sidebar-nav">
                <ul class="nav">
                    <li class="nav-item">
                        <a class="nav-link" href="index.php?dashboard"><i class="icon-graph"></i> Dashboard </a>
                    </li>

                                         <li class="nav-item">
                        <a class="nav-link" href="interns.php"><i class="icon-list"></i> Interns </a>
                    </li>
                      
                    

                                                         </ul>
            </nav>
            <button class="sidebar-minimizer brand-minimizer" type="button"></button>
        </div>
        <main class="main">
              
<div class="card">
	<div class="card-header">
		<h4><strong> Man, Spider (11111) Attachment @ Madaraka Primary School</strong>
						
		</h4>
	</div>
</div>
<div class="container-fluid">
	<div class="row">
		<div class="col-sm-12">
							<div class="card">
					<div class="card-header">Assessment Scores</div>
					<div class="card-body">
						<div class="row">
							<div class="col-sm-6 col-lg-6">
								<div class="card text-white bg-primary">
									<div class="card-body">
										<p class="text-center">Host Supervisor Assessment</p>
										<h1 class="text-center">
											50/50
										</h1>
									</div>
								</div>
							</div>
							<div class="col-sm-6 col-lg-6">
								<div class="card text-white bg-primary">
									<div class="card-body">
										<p class="text-center">Faculty Supervisor Assessment</p>
										<h1 class="text-center">
											50/50
										</h1>
									</div>
								</div>
							</div>

						</div>
					</div>
				</div>

							<div class="card">
				<div class="card-header">
					Assigned Faculty Supervisor
									</div>
				<div class="card-body">
										<div class="row">
						<div class="col">
							<div class="list-group-item">
								<p class="mb-1">Austin Waswa<br>
									<small>Supervisor Name</small></p>							
								</div>
							</div>
							<div class="col">
								<div class="list-group-item">
									<p class="mb-1">waswagmail,com<br>
										<small>Supervisor's Email</small></p>							
									</div>
								</div>
								
									<div class="col">
										<div class="list-group-item">
											<p class="mb-1">Wed, 22rd Dec, 2021<br>
												<small>Date Allocated</small></p>							
											</div>
										</div>
																			</div>
																	</div>
							</div>
							
							<div class="card">
								<div class="card-header">
									Host Supervisor
																			
										
									<a href="asses.php" class="btn btn-primary pull-right">View Assessment</a>
																																			</div>
								<div class="card-body">
									<div class="row">
										<div class="col">
											<div class="list-group-item">
												<p class="mb-1"><?php echo $host_name; ?><br>
													<small>Host Supervisor Name</small></p>						
												</div>
											</div>
											<div class="col">
												<div class="list-group-item">
													<p class="mb-1">HeadTeacher<br>
														<small>Host Supervisor Designation</small></p>							
													</div>
												</div>
												<div class="col">
													<div class="list-group-item">
														<p class="mb-1">Paloma.maina@gmail.com<br>
															<small>Attachment Co-ordinator Email Address</small></p>					
														</div>
													</div>
													<div class="col">
														<div class="list-group-item">
															<p class="mb-1">+25400000004<br>
																<small>Attachment Co-ordinator Phone</small></p>						
															</div>
														</div>
													</div>
												</div>
											</div>

											<div class="card">
												<div class="card-header">
													Student Details
												</div>
												<div class="card-body">
													<div class="row">
														<div class="col">
															<div class="list-group-item">
																<p class="mb-1">Man, Spinder<br>
																	<small>Student Name</small></p>						
																</div>
															</div>
															<div class="col">
															<div class="list-group-item">
																<p class="mb-1">111111<br>
																	<small>Student Number</small></p>						
																</div>
															</div>
															<div class="col">
															<div class="list-group-item">
																<p class="mb-1">spinderman @gmail.com<br>
																	<small>Email Address</small></p>						
																</div>
															</div>
															<div class="col">
															<div class="list-group-item">
																<p class="mb-1">0707070707<br>
																	<small>Phone Number</small></p>						
																</div>
															</div>
																<div class="col">
															<div class="list-group-item">
																<p class="mb-1">Bachelor of Business Information Technology (BBIT)<br>
																	<small>Course</small></p>						
																</div>
															</div>



														</div>
													</div>
												</div>																	
												<div class="card">
													<div class="card-header">
														Student Attachment Details
													</div>
													<div class="card-body">
														<div class="row">
															<div class="col">
																<div class="list-group-item">
																	<p class="mb-1">Madaraka Primary School<br>
																		<small>Organization Name</small></p>							
																	</div>
																</div>
																<div class="col">
																	<div class="list-group-item">
																		<p class="mb-1">West Madaraka Primary School<br>
																			<small>Organization Branch</small></p>							
																		</div>
																	</div>
																	<div class="col">
																		<div class="list-group-item">
																			<p class="mb-1">Madaraka<br>
																				<small>General Area</small></p>							
																			</div>
																		</div>
																		<div class="col">
																			<div class="list-group-item">
																				<p class="mb-1">Madaraka road<br>
																					<small>Road/Street</small></p>						
																				</div>
																			</div>






																				<div class="w-100"></div>
																				<div class="col">
																					<div class="list-group-item">
																						<p class="mb-1">Bachelor of Business Information Technology<br>
																							<small>Course</small></p>							
																						</div>
																					</div>
																					<div class="col">
																						<div class="list-group-item">
																							<p class="mb-1">Service Based Learning<br>
																								<small>Attachment Type</small></p>							
																							</div>
																						</div>
																						<div class="col">
																							<div class="list-group-item">
																								<p class="mb-1">Wed, 22nd Dec, 2021 - Thu, 25th Feb, 2022<br>
																									<small>Actual Attachment Period</small></p>						
																								</div>
																							</div>
																							<div class="col">
																								<div class="list-group-item">
																									<p class="mb-1">40<br>
																										<small>Hours Per Week</small></p>						
																									</div>
																								</div>
																								<div class="col">
																									<div class="list-group-item">
																										<p class="mb-1">MON,TUE,WED,THU,FRI<br>
																											<small>Working Days</small></p>							
																										</div>
																									</div>
																									<div class="w-100"></div>
																									<div class="col">
																										<div class="list-group-item">
																											<p class="mb-1">Default Attachment Period<br>
																												<small>University Attachment Period</small></p>							
																											</div>
																										</div>
																										<div class="col">
																											<div class="list-group-item">
																												<p class="mb-1">Madaraka Primary School is a public local school. It accommodates pupils from grade 1 to class 8.<br>
																													<small>General Description</small></p>							
																												</div>
																											</div>

																											<div class="col">
																												<div class="list-group-item">
																													<p class="mb-1">Kenya<br>
																														<small>Country</small></p>							
																													</div>
																												</div>
																												<div class="col">
																													<div class="list-group-item">
																														<p class="mb-1">Nairobi<br>
																															<small>County</small></p>							
																														</div>
																													</div>
																													<div class="col">
																														<div class="list-group-item">
																															<p class="mb-1">Nairobi West<br>
																																<small>Constituency</small></p>						
																															</div>
																														</div>					</div>
																													</div>
																												</div>
																											</div>
																										</div>









																										<div class="row">
																											<div class="col-sm-12">
																												<div class="card">
																													<div class="card-header">
																														Internship Objectives
																													</div>
																													<div class="card-body">
																														Teaching .
Planning of lessons . 
Management of duties .
Supervising Students
Collaborating other teachers
																													</div>
																												</div>
																											</div>
																										</div>
																										<div class="row">
																											<div class="col-sm-12">
																												<div class="card">
																													<div class="card-header">
																														Internship Skills
																													</div>
																													<div class="card-body">
																														Scheduling .
Writing (project development) .
Communication skills .
Teamwork skills .
Creativity .
																													</div>
																												</div>
																											</div>
																										</div>
																										<div class="row">
																											<div class="col-sm-12">
																												<div class="card">
																													<div class="card-header">
																														Internship Training Opportunities
																													</div>
																													<div class="card-body">
																														Interacting with children and fellow teacher this helps me with my communicating and teamwork skills.
																													</div>
																												</div>
																											</div>
																										</div>
																									</div>

<footer class="app-footer">
        <span>&copy; <a href="#" target="_blank">BBIT GROUP A</a> 2021.</span>
    </footer>

    <script src="scripts/app.js"></script>
    
    <script src="scripts/popper.min.js"></script>
    <script src="scripts/bootstrap.min.js"></script>
    <script src="scripts/pace.min.js"></script>
    <script src="scripts/Chart.min.js"></script>
    <script src="scripts/datatables.min.js"></script>
    <script src="scripts/select2.min.js"></script>
    <script src="scripts/.easy-autocomplete.js"></script>
    <script src="scripts//moment.js"></script>
    <script src="scripts/daterangepicker.js"></script>
    <script src="scripts/custom.js"></script>
    																									<script>
																										$(function () {

																											$( "#staff_member_id" ).change(function() {
																												var selectedValue = $("#staff_member_id").val();
																												if(selectedValue){
																							//Go fetch workload for faculty supervisor
																							$.get( ".",{staff_member_id:selectedValue}, function( data ) {
																								var pload = "<strong>Supervisor Workload</strong><br>";
																								$.each( data, function( key, value ) {								
																									pload += "<li>"+key+" ("+value+")"+"</li>"
																								});
																								$( "#workload" ).html( "<ul>"+pload+"</ul>" );	
																							});
																						}
																					});

																										});
																									</script>
																									</body>

</html>